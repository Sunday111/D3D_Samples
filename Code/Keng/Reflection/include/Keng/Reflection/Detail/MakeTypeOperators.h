...
namespace keng::reflection::detail
{
	namespace has_operator
	{
		...
		template<typename T>
		struct has_equals_operator<T, decltype(void(std::declval<const T&>() == std::declval<const T&>()))> : std::true_type {};
	...
	constexpr TypeOperators MakeTypeOperators() {
		return TypeOperators {
			...
			MakeCopyAssignOperator<T>(),
			MakeMoveAssignOperator<T>(),
			MakeEqualsOperator<T>(),
			MakeDestructor<T>()
		};
	}
}
